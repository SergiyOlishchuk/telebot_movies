...
@dp.callback_query_handler(lambda c: c.data and c.data == 'movie special', state=None)
async def process_callback_movie_special(call: types.CallbackQuery):
    await bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'{call.message.text}\n\nФільм за всіма параметрами')

    await FSMMovies.genres.set()
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('genre'), state=FSMMovies.genres)
async def process_genres_special(call: types.CallbackQuery, state: FSMContext):
...
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('year'), state=FSMMovies.years)
async def process_years_special(call: types.CallbackQuery, state: FSMContext):
    ...
    if call_year == 'ready':
        ...
        async with state.proxy() as data:
            data['years'] = selected_years[user_id]
        
        async with state.proxy() as data:  
            selected_movies[user_id] = db.get_random_movies(1, genres=tuple(data['genres']), years=tuple(data['years']))[0]
...
async def process_years(call: types.CallbackQuery):
    ...
    if call_year == 'ready':
        ...
        if len(selected_years[user_id]) > 0:
            ...
            await bot.send_message(call.message.chat.id, 'Ви не вибрали роки(')
